...
//     // 6. Update user table for latest score + total time taken
//     await client.query(
//       `UPDATE users SET score=$1, total=$2, details=$3, time_taken_seconds=$4 WHERE id=$5`,
//       [result.score, result.total, JSON.stringify(result.details), timeTakenSeconds, user.id]
//     );

//     await client.query('COMMIT');
//     res.json({ attemptId, ...result, timeTakenSeconds });
//   } catch (err: any) {
//     await client.query('ROLLBACK');
//     console.error('Submit error:', err.message);
//     res.status(500).json({ error: 'Submission failed', details: err.message });
